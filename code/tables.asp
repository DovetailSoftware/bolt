...
<% var name_filter = Request("type_name");
	...
	if(rsTables.EOF) {
		if(id_filter >= 0) {
			rw("<h3>No matches for table or view id = " + id_filter + "</h3>");
		} else if(rangeQuery==true){
			rw("<h3>No matches for tables or views in the given range (" + minRange + "-"+ maxRange + ")</h3>");
		} else {
			rw("<h3>No matches for table or view name starting with '" + name_filter + "'</h3>");
		}
	...
	//Page Header:
	if(custom_filter==1) {
		rw("<h3>Custom Tables/Views</h3>");
	} else if(rangeQuery == true) {
		rw("<h3>Tables/Views in ID range " + minRange + "-" + maxRange + "</h3>");
	} else {
		rw("<h3>Tables/Views</h3>");
	}
